...
const MentorApplication = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "experience") {
      setFormData({ ...formData, [name]: value.replace(/\D/g, "") }); // Only numbers
    } else {
      setFormData({ ...formData, [name]: value });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://your-backend-url.com/mentor-apply", {
        method: "POST",
